...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      toast.error('Failed to load dashboard data. Check connection to backend.');
  ...
  if (!dashboardData) {
    return (
      <div className={`flex flex-col items-center justify-center min-h-screen gap-4 transition-colors ${
        darkMode ? 'bg-black' : 'bg-gray-50'
      }`}>
        <p className={darkMode ? 'text-gray-400' : 'text-gray-600'}>Failed to load dashboard data. Ensure the backend is reachable and you are logged in.</p>
        <button
  ...
  // Use only backend data for recent activities (no mock fallback)
            ...
            <div className={`reveal-up delay-200 ${cardBg} ${cardBorder} rounded-2xl overflow-hidden shadow-lg h-fit border`}>
              ...
              <div className={`divide-y ${darkMode ? 'divide-white/5' : 'divide-gray-200'}`}>
                {recentActivities.length === 0 ? (
                  <div className={`p-4 ${textSecondary} text-sm`}>No recent activity</div>
